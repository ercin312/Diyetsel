...
class PdfReport {
  ...
  static pw.Widget _sectionTitle(String text) => pw.Padding(
        ...
        child: pw.Text(text, style: pw.TextStyle(color: _orange, fontWeight: pw.FontWeight.bold, fontSize: 13)),
      );
  ...
  static pw.Widget _metricRow(String label, String value) => pw.Padding(
        ...
        child: pw.Row(
          ...
          children: [
            pw.Text(label, style: pw.TextStyle(color: _muted)),
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        build: (ctx) => [
          ...
          pw.Text(report.user.displayName, style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
          pw.Text(range, style: pw.TextStyle(color: _muted)),
  ...
  static Future<void> shareClientReport({
    ...
    doc.addPage(
      pw.MultiPage(
        ...
        build: (ctx) => [
          pw.Text('Tam geçmiş raporu', style: pw.TextStyle(fontSize: 18, fontWeight: pw.FontWeight.bold)),
          pw.Text(user.displayName, style: pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
          pw.Text(user.email),
          pw.SizedBox(height: 16),
          _sectionTitle('Diyet planı'),
          if (plan == null)
            pw.Text('Atanmış plan yok.')
          else
            ...plan.days.map(
              (day) => pw.Column(
                ...
                children: [
                  pw.SizedBox(height: 8),
                  pw.Text(DateFormat('EEEE d MMM', 'tr').format(day.date), style: pw.TextStyle(fontWeight: pw.FontWeight.bold)),
      ...
      pw.Page(
        build: (ctx) => pw.Column(
          ...
          children: [
            ...
            pw.Text(
              'Ciro (tahsil): ₺${payments.where((p) => p.status == PaymentStatus.paid).fold<double>(0, (s, p) => s + p.amount).toStringAsFixed(0)}',
            ),
          ],
        ),
      ),
